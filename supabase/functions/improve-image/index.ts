...
serve(async (req) => {
  ...
  try {
    ...
    const improvedImageUrl = data.choices?.[0]?.message?.images?.[0]?.image_url
      ?.url;
    ...
    try {
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        if (assetResponse.ok) {
          ...
          if (!uploadError) {
            const { data: publicUrl } = assetClient.storage.from("assets")
      ...
      console.warn(
        "Failed to persist improved image to assets bucket",
        storageError,
      );
    }
