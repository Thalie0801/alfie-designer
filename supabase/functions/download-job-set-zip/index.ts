...
function derivePublicIdFromUrl(url?: string): string | undefined {
  ...
  console.log(
    `[derivePublicId] Derived '${derivedId}' from URL: ${
      url.substring(0, 100)
    }...`,
  );
  return derivedId;
...
function buildOverlayUrl(slide: any): string | null {
  ...
  const publicId = slide.cloudinary_public_id ||
    derivePublicIdFromUrl(slide.cloudinary_url);

  console.log(
    `[buildOverlayUrl] slide=${slide.id} cloudName=${cloudName} publicId=${publicId} has_text=${!!slide
      .text_json}`,
  );

  if (!cloudName) {
    console.warn(
      `[buildOverlayUrl] ❌ Missing cloudName for slide ${slide.id}`,
    );
    return null;
  }
  ...
  if (!slide.text_json) {
    console.warn(
      `[buildOverlayUrl] ❌ Missing text_json for slide ${slide.id}`,
    );
    return null;
  }
  ...
  const cleanBullets = (bullets || []).map((b: string) => cleanText(b, 80))
    .slice(0, 6);

  if (!cleanTitle || cleanTitle.trim() === "") {
    console.warn(
      `[buildOverlayUrl] ❌ Empty title after cleaning for slide ${slide.id}`,
    );
    return null;
  }

  console.log(
    `[buildOverlayUrl] ✅ Building overlay for slide ${slide.id}, title="${
  ...
  overlays.push(
    `l_text:Arial_${titleSize}_bold:${
      encodeCloudinaryText(cleanTitle)
    },co_rgb:FFFFFF,g_north,y_${titleY},w_${Math.round(dims.w * 0.9)},c_fit`,
  );
  ...
  if (cleanSubtitle && cleanSubtitle.trim() !== "") {
    ...
    overlays.push(
      `l_text:Arial_${subSize}:${
        encodeCloudinaryText(cleanSubtitle)
      },co_rgb:E5E7EB,g_south,y_${subY},w_${Math.round(dims.w * 0.84)},c_fit`,
    );
  }
  ...
  if (cleanBullets.length > 0) {
    ...
    cleanBullets.forEach((b: string, i: number) => {
      if (b.trim() !== "") {
        overlays.push(
          `l_text:Arial_${bulletSize}:${
            encodeCloudinaryText("• " + b)
          },co_rgb:FFFFFF,g_center,y_${startY + i * step},w_${
            Math.round(dims.w * 0.8)
          },c_fit`,
        );
      }
    });
  ...
  const finalUrl =
    `https://res.cloudinary.com/${cloudName}/image/upload/${baseTransform}${overlayTransforms}/${publicId}`;

  console.log(
    `[buildOverlay] ✅ Built overlay URL for slide ${slide.id} (${format})`,
  );
  return finalUrl;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const queryCarouselId = requestUrl.searchParams.get("carousel_id") ||
      requestUrl.searchParams.get("carouselId");
    const queryOrderId = requestUrl.searchParams.get("order_id") ||
    ...
    if (!carouselId && !orderId) {
      throw new Error("Missing carouselId or orderId");
    }
    ...
    console.log(
      `[download-zip] User ${user.id} requesting ZIP for carousel=${carouselId}, order=${orderId}`,
    );
    ...
    let query = supabase
      .from("library_assets")
      .select(
        "id, slide_index, cloudinary_url, cloudinary_public_id, text_json, format, metadata, carousel_id, order_id",
      )
    ...
    console.log(
      `[download-zip] Found ${slides.length} slides for ${
        orderId ? "order" : "carousel"
      }`,
    );
    ...
    for (const slide of slides) {
      ...
      if (!imageUrl) {
        ...
        console.log(
          `[download-zip] No overlay for slide ${slide.id}, using base image`,
        );
      }
      ...
      try {
        const overlayIndicator = imageUrl.includes("l_text:")
          ? "🎨 WITH OVERLAY"
          : "📦 base";
        console.log(
          `[download-zip] Downloading slide ${slide.slide_index} ${overlayIndicator}: ${
            imageUrl.substring(
              0,
              120,
            )
        ...
        if (!response.ok) {
          console.warn(
            `[download-zip] ⚠️ Overlay failed (${response.status}), trying base...`,
          );
          ...
          if (slide.cloudinary_url) {
            ...
            if (fallbackResp.ok) {
              ...
              console.log(
                `[download-zip] ✅ Added ${filename} via base fallback`,
              );
              successCount++;
              continue;
            }
        ...
        zip.file(filename, new Uint8Array(arrayBuffer));
        console.log(
          `[download-zip] ✅ Added ${filename} (${arrayBuffer.byteLength} bytes) ${
            imageUrl.includes("l_text:") ? "🎨 WITH OVERLAY" : "📦 base"
          }`,
        );
        successCount++;
      } catch (err) {
        console.error(
          `[download-zip] ❌ Error fetching slide ${slide.id}:`,
          err,
        );
        failureCount++;
      }
    ...
    console.log(
      `[download-zip] Summary: ${successCount} successful, ${failureCount} failed`,
    );

    if (successCount === 0) {
      throw new Error(
        "No slides could be downloaded - all images failed to load",
      );
    }
