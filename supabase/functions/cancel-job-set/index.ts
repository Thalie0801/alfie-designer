...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      "[CancelJobSet] Request from user:",
      user.id,
      "for job_set:",
      jobSetId,
    );
    ...
    if (jobSet.user_id !== user.id) {
      ...
      if (!brand || brand.user_id !== user.id) {
        console.error(
          `[CancelJobSet] ⛔ User ${user.id} tried to cancel job_set ${jobSetId} from brand ${jobSet.brand_id}`,
        );
        return new Response(
          JSON.stringify({
    ...
    if (["canceled", "done", "failed"].includes(jobSet.status)) {
      console.log(
        "[CancelJobSet] Job set already in terminal state:",
        jobSet.status,
      );
      return new Response(
        JSON.stringify({
    ...
    if (remainingCount > 0) {
      ...
      console.log(
        `[CancelJobSet] Refunding ${remainingCount} visuals for brand ${jobSet.brand_id} in period ${periodYYYYMM}`,
      );

      // Rembourser dans counters_monthly (système unifié)
      const { error: refundMonthlyError } = await supabaseAdmin.rpc(
      ...
      if (refundMonthlyError) {
        console.error(
          "[CancelJobSet] Failed to refund monthly counters:",
          refundMonthlyError,
        );
      ...
      const { error: refundBrandError } = await supabaseAdmin.rpc(
        "refund_brand_quotas",
        {
          p_brand_id: jobSet.brand_id,
      ...
      if (refundBrandError) {
        console.error(
          "[CancelJobSet] Failed to refund brand quotas:",
          refundBrandError,
        );
