...
serve(async (req) => {
  ...
  if (!INTERNAL_FN_SECRET) {
    console.error("Missing INTERNAL_FN_SECRET for send-confirmation-email");
  ...
  const providedSecret = req.headers.get("x-internal-fn-secret");
  if (providedSecret !== INTERNAL_FN_SECRET) {
