...
import {
  ...
  compositeSlide,
} from "../_shared/imageCompositor.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: job, error: jobErr } = await supabase
      .from("jobs")
      .select(
        "*, job_sets!inner(brand_id, user_id, master_seed, constraints, status)",
      )
      .eq("status", "queued")
    ...
    console.log(
      `📋 [Worker] Processing job ${job.id} (index: ${job.index_in_set})`,
    );
    ...
    if (["canceled", "failed"].includes(job.job_sets.status)) {
      console.log(
        `[Worker] ⏭️  Skipping job ${job.id} - job_set is ${job.job_sets.status}`,
      );
      return new Response(
        JSON.stringify({ message: "Job set canceled or failed, skipping" }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
    ...
    if (job.index_in_set === 0 && !isKeyVisualCheck) {
      console.warn(
        "⚠️ [Worker] Slide 0 should be key_visual but role is:",
        job.metadata?.role,
      );
    }
    ...
    if (lockErr || !lockedJob) {
      console.log(
        `[Worker] Job ${job.id} already taken by another worker, skipping`,
      );
    ...
    const masterSeedStr = brandSnapshot?.master_seed ||
      job.job_sets.master_seed;
    ...
    if (masterSeedStr) {
      ...
      if (isKeyVisual) {
        ...
        console.log(
          `[Worker] Generating KEY VISUAL with master seed ${
            seed?.slice(0, 12) || "none"
          }...`,
        );
      } else {
        ...
        console.log(
          `[Worker] Generating VARIANT #${job.index_in_set} with derived seed ${
            seed?.slice(0, 12) || "none"
          }...`,
        );
      }
      ...
      console.log(
        `[Worker] No master seed available, generating without seed control`,
      );
    }
    ...
    console.log(
      "📝 [Worker] Step 1: Building overlayText from structured slideContent...",
    );
    ...
    const metaSlideContent = job.metadata?.slideContent ??
      (
        legacy.title || legacy.subtitle || legacy.punchline || legacy.bullets ||
          legacy.cta || legacy.kpis
          ? {
    ...
    if (!metaSlideContent) {
      console.error(
        "❌ [Worker] Missing slideContent in job metadata - plan was not executed properly",
      );
    ...
    console.log(
      `[Worker] Using structured slideContent: type=${slideContent.type}, title="${slideContent.title}"`,
    );
    ...
    checkTimeout();
    console.log(
      "🖼️ [Worker] Step 2: Generating background image WITHOUT text...",
    );
    ...
    while (retryAttempt <= maxRetries) {
      try {
        ...
        if (retryAttempt > 0) {
          console.log(
            `🔄 [Worker] Retry attempt ${retryAttempt}/${maxRetries} for AI generation...`,
          );
        }
        ...
        if (!gatewayResp.ok) {
          ...
          if (statusCode >= 400 && statusCode < 500) {
            console.error(
              `❌ [Worker] AI gateway client error ${statusCode}: ${errText}`,
            );
          ...
          if (retryAttempt < maxRetries) {
            console.warn(
              `⚠️ [Worker] AI gateway error ${statusCode}, will retry...`,
            );
            retryAttempt++;
        ...
        const imgUrl = gatewayJson?.choices?.[0]?.message?.images?.[0]
          ?.image_url?.url;
        ...
        console.log(
          "✅ [Worker] Background generated successfully" +
        ...
        if (
          ...
          console.warn(
            `⚠️ [Worker] Network/timeout error, will retry: ${err.message}`,
          );
          retryAttempt++;
    ...
    if (backgroundImageData) {
      console.log(
        "📦 [Worker] AI response keys:",
        Object.keys(backgroundImageData),
      );
      const dataStr = JSON.stringify(backgroundImageData);
      console.log(
        "📦 [Worker] AI payload preview:",
        dataStr ? dataStr.slice(0, 500) : "null",
      );
    }
    ...
    function pickImage(
      ...
      const url = gen.imageUrl || gen.url ||
      ...
      const b64 = gen.base64 || gen.b64 || gen.b64_json ||
        gen.image?.base64 || gen.images?.[0]?.base64 ||
        gen.images?.[0]?.b64_json;
    ...
    if (backgroundImageErr || !pickedBackground) {
      if (!pickedBackground) {
        console.error(
          "❌ [Worker] Could not extract background image. Available keys:",
          backgroundImageData ? Object.keys(backgroundImageData) : "null",
        );
        const dataStr = backgroundImageData
          ? JSON.stringify(backgroundImageData)
          : "null";
        console.error(
          "📦 [Worker] Full payload (first 500 chars):",
          dataStr && typeof dataStr === "string"
            ? dataStr.slice(0, 500)
            : dataStr,
        );
      } else {
        console.error(
          "❌ [Worker] Background image generation failed:",
          backgroundImageErr,
        );
      }
      ...
      console.error(
        ...
        backgroundImageErr?.message || "no_image",
      );
      return new Response(
        JSON.stringify({
          success: false,
    ...
    if (pickedBackground.kind === "url") {
      ...
      const tempFileName =
        `carousel/${job.job_set_id}/temp_bg_${job.index_in_set}_${Date.now()}.png`;
      const { error: tempUploadError } = await supabase.storage
        .from("media-generations")
        .upload(tempFileName, tempBytes, {
    ...
    checkTimeout();
    console.log(
      "🎨 [Worker] Step 5: Compositing background + text via Cloudinary...",
    );

    // Extract primary and secondary colors from brand for tint
    const primaryColor = brandSnapshot?.primary_color ||
      brandSnapshot?.palette?.[0];
    const secondaryColor = brandSnapshot?.secondary_color ||
      brandSnapshot?.palette?.[1];
    ...
    let composedResult:
      | { url: string; bgPublicId: string; svgPublicId: string }
      | null = null;
    ...
    while (compositionAttempts < maxCompositionAttempts) {
      try {
        compositionAttempts++;
        console.log(
          `🎨 [Worker] Composition attempt ${compositionAttempts}/${maxCompositionAttempts}`,
        );
        ...
        const errorMessage = compositionError instanceof Error
          ? compositionError.message
          : String(compositionError);
        console.error(
          `❌ [Worker] Composition attempt ${compositionAttempts} failed:`,
          errorMessage,
        );
        ...
        if (compositionAttempts >= maxCompositionAttempts) {
          ...
          console.warn(
            "⚠️ [Worker] Using background-only fallback (no text overlay)",
          );
          ...
          await supabase
            ...
            .update({
              metadata: {
                ...job.metadata,
                composition_warning:
                  "Text overlay failed, using background only",
    ...
    checkTimeout();
    const imageSource = compositionFailed
      ? "background (fallback)"
      : "composed image";
    ...
    if (!imageResponse.ok) {
      throw new Error(
        `Failed to download ${imageSource}: ${imageResponse.status}`,
      );
    }
    const finalImageBytes = new Uint8Array(await imageResponse.arrayBuffer());
    console.log(
      "✅ [Worker] Image downloaded:",
    ...
    const fileName =
      `carousel/${brandId}/${job.job_set_id}/slide_${job.index_in_set}_${Date.now()}.png`;
    ...
    if (!isKeyVisual && job.index_in_set > 0) {
      ...
      if (keyVisualAsset?.meta?.public_url) {
        ...
        console.log(
          `[Worker] Using key_visual as reference for style consistency: ${referenceImageUrl}`,
        );
      }
    }
    ...
    if (!referenceImageUrl && job.job_sets.style_ref_url) {
      ...
      console.log(
        `[Worker] Using style_ref_url as reference: ${referenceImageUrl}`,
      );
    }
    ...
    console.log(
      `[Worker] Coherence score: ${coherenceScore.total}/100`,
      coherenceScore.breakdown,
    );
    ...
    console.log(
      `[Worker] Using effective coherence threshold: ${coherenceThresholdEffective} (with brand tint)`,
    );

    if (
      coherenceScore.total < coherenceThresholdEffective && retryCount === 0 &&
      composedResult
    ) {
      // ONE TIME retry with stronger tint (only if composition succeeded initially)
      console.log(
        `[Worker] Low coherence (${coherenceScore.total}), retrying with stronger tint...`,
      );
      ...
      if (retryImageResponse.ok) {
        const retryImageBytes = new Uint8Array(
          await retryImageResponse.arrayBuffer(),
        );
        const retryFileName =
          `carousel/${brandId}/${job.job_set_id}/slide_${job.index_in_set}_retry_${Date.now()}.png`;
        ...
        if (!retryUploadError && retryUploadData) {
          ...
          console.log(
            `[Worker] Retry coherence score: ${retryCoherenceScore.total}/100`,
          );

          // Use retry image if better
          if (retryCoherenceScore.total > coherenceScore.total) {
            console.log(
              `[Worker] Using retry image (better score: ${retryCoherenceScore.total} > ${coherenceScore.total})`,
            );
            // Cleanup old upload
        ...
        await cleanupCloudinaryResources({
          bgPublicId: strongerTint.bgPublicId,
          svgPublicId: strongerTint.svgPublicId,
        });
      }
    }
    ...
    if (assetErr || !asset) {
      console.error(
        "❌ [Worker] Insert into media_generations failed:",
        assetErr,
      );
      ...
      await supabase
        ...
        .update({
          ...
          error: `Asset creation failed: ${
            assetErr?.message || "No asset returned"
          }`,
          finished_at: new Date().toISOString(),
        })
        .eq("id", job.id);
    ...
    console.log(
      `💰 [Worker] Quota consumption confirmed for brand ${job.job_sets.brand_id} (1 visual)`,
    );
    ...
    console.log(
      `✅ [Worker] Job ${job.id} marked as succeeded with asset_id: ${asset.id}`,
    );
    ...
    if (isKeyVisual && asset) {
      await supabase
        ...
        .eq("id", job.job_set_id);
      console.log(
        `[Worker] Updated job_set ${job.job_set_id} with style_ref_url`,
      );
    }
    ...
    console.log(
      `[Worker] Job ${job.id} completed successfully (set status: ${finalStatus}, coherence: ${coherenceScore.total}/100)`,
    );
    ...
    if (jobIdForCleanup) {
      try {
        console.log(
          `🔄 [Worker] Attempting to mark job ${jobIdForCleanup} as failed...`,
        );
