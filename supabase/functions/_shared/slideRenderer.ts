...
export async function renderSlideToSVG(
  ...
  const rawFontFamily = brandSnapshot.fonts?.default ||
  ...
  for (const layer of template.textLayers) {
    ...
    if (!hexRegex.test(textColor)) {
      console.warn(
        `[slideRenderer] Invalid color "${textColor}" for layer ${layer.id}, using brand color`,
      );
  ...
  if (openTags !== closeTags) {
    console.error("[slideRenderer] Unbalanced text tags:", {
      openTags,
      closeTags,
    });
...
function getTextForLayer(layer: TextLayer, content: SlideContent): string {
  switch (layer.id) {
    ...
    case "punchline":
      return content.punchline || "";
    case "cta":
      return content.cta || "";
    case "cta_primary":
      return content.cta_primary || "";
    case "cta_secondary":
      return content.cta_secondary || "";
    case "note":
      return content.note || "";
    case "badge":
...
function adjustForContrast(
  ...
  if (bgLum > 0.5) {
    while (
      calculateContrast(`#${toHex(r)}${toHex(g)}${toHex(b)}`, background) <
        targetRatio
    ) {
      r = Math.max(0, r - 10);
      g = Math.max(0, g - 10);
