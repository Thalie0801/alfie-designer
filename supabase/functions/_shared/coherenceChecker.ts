...
export async function checkCoherence(
  ...
  for (const brandColor of constraints.palette) {
    const closestDelta = Math.min(
      ...dominantColors.map((imgColor) =>
        calculateDeltaE(brandColor, imgColor)
      ),
    );
  ...
  if (publicId) {
    try {
      ...
      if (CLOUD_NAME && API_KEY && API_SECRET) {
        ...
        const url =
          `https://api.cloudinary.com/v1_1/${cloudName}/resources/image/upload/${encodedPublicId}?colors=true`;
        ...
        if (response.ok) {
          ...
          if (data.colors && Array.isArray(data.colors)) {
            const dominantColors = data.colors.slice(0, 5).map((c: any) =>
              c[0]
            );
            console.log(
              "[coherenceChecker] Extracted colors from Cloudinary:",
              dominantColors,
            );
      ...
      console.warn(
        "[coherenceChecker] Failed to extract colors from Cloudinary:",
        err,
      );
    }
  }
...
async function detectTextInImage(imageUrl: string): Promise<boolean> {
  try {
    ...
    if (!LOVABLE_API_KEY) {
      console.warn(
        "[coherenceChecker] LOVABLE_API_KEY not set, assuming no text",
      );
      return false;
    }
              ...
              content: [
                {
                  type: "text",
                  text:
                    "Does this image contain any visible text, letters, words, or typography? Answer ONLY with YES or NO, nothing else.",
                },
...
async function calculateStyleSimilarity(
  ...
  console.log("[coherenceChecker] Style similarity check (MVP):", {
    imageA,
    imageB,
  });
  return 85;
