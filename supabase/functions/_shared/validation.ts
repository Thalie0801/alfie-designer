...
export const generateVideoSchema = z.object({
  prompt: z.string().min(3, "Prompt trop court").max(
    5000,
    "Prompt limité à 5000 caractères",
  ),
...
export const alfieMessageSchema = z.object({
  ...
  messages: z.array(
    z.object({
      ...
      content: z.string().min(1, "Message vide").max(
        10000,
        "Message trop long",
      ),
      imageUrl: z.string().url("URL image invalide").optional(),
...
export const createVideoSchema = z.object({
  title: z.string().min(1, "Titre requis").max(
    200,
    "Titre limité à 200 caractères",
  ),
  duration: z.number().int().min(5, "Durée minimale 5s").max(
    60,
    "Durée maximale 60s",
  ),
...
export const deliverableSchema = z.object({
  ...
  objective: z.string().min(3, "Objectif trop court").max(
    500,
    "Objectif limité à 500 caractères",
  ).optional(),
...
export const CheckoutSchema = z.object({
  ...
  affiliate_ref: z.string().uuid("Référence affilié invalide").optional().or(
    z.literal(""),
  ),
  brand_name: z.string()
    .trim()
    ...
    .regex(
      /^[a-zA-Z0-9\s\-_'\.]+$/,
      "Nom de marque contient des caractères non autorisés",
    )
    .optional()
    .or(z.literal("")),
});
