...
export async function userHasAccess(authHeader: string | null) {
  const ENFORCE =
    (Deno.env.get("AUTH_ENFORCEMENT") ?? "on").toLowerCase() === "on";
  ...
  console.log(
    `[AccessControl] Checking access for: ${user.email} | Roles: ${
      userRoles.join(",")
    } | isVip: ${isVip} | isAdmin: ${isAdmin}`,
  );

  if (isVip || isAdmin) {
    console.log(
      `[AccessControl] ✅ Access granted via ${
        isVip ? "VIP" : "ADMIN"
      } role from database`,
    );
    return true;
