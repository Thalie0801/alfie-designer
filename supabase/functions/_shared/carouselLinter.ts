...
export function lintCarousel(
  ...
  const t3Content = JSON.stringify(
    slides.find((s) => s.type === "solution") || {},
  ).toLowerCase();
  const t5Content = JSON.stringify(slides.find((s) => s.type === "cta") || {})
    .toLowerCase();

  if (
    !t3Content.includes(promiseKeyword) && !t5Content.includes(promiseKeyword)
  ) {
  ...
  if (heroSlide?.cta_primary && ctaSlide?.cta_primary) {
    if (heroSlide.cta_primary.trim() !== ctaSlide.cta_primary.trim()) {
      errors.push(
        `R2: CTA différent entre Hero ("${heroSlide.cta_primary}") et CTA ("${ctaSlide.cta_primary}")`,
      );
    }
  } else {
  ...
  slides.forEach((slide, index) => {
    ...
    if (!hasTerminology) {
      warnings.push(
        `R3: Slide ${index + 1} (${slide.type}) sans terme du glossaire`,
      );
    }
    ...
    globals.banned.forEach((banned) => {
      if (slideText.includes(banned.toLowerCase())) {
        errors.push(
          `R3: Slide ${
            index + 1
          } (${slide.type}) contient le mot banni "${banned}"`,
        );
      }
    });
  });
  ...
  slides.forEach((slide, index) => {
    ...
    fields.forEach((field) => {
      ...
      if (field.includes("!!")) {
        warnings.push(
          `R4: Slide ${index + 1} contient des points d'exclamation multiples`,
        );
      }
      ...
      words.forEach((word) => {
        if (
          ...
          warnings.push(
            `R4: Slide ${index + 1} contient du texte en MAJUSCULES: "${word}"`,
          );
        }
      });
    });
  ...
  if (problemSlide?.bullets && solutionSlide?.bullets) {
    if (problemSlide.bullets.length > solutionSlide.bullets.length) {
      warnings.push(
        "R7: Plus de problèmes que de solutions (mapping incomplet)",
      );
    }
    ...
    problemSlide.bullets.forEach((bullet, i) => {
      ...
      if (
        lowerBullet.includes("solution") || lowerBullet.includes("résoudre") ||
        lowerBullet.includes("grâce à")
      ) {
  ...
  slides.forEach((slide, index) => {
    ...
    hyperboles.forEach((hyp) => {
      if (slideText.includes(hyp)) {
        warnings.push(
          `R8: Slide ${index + 1} contient une hyperbole: "${hyp}"`,
        );
      }
    });
  });
...
function checkCharLimit(
  ...
  if (len < limit.min) {
    warnings.push(
      `R6: Slide ${slideIndex} ${fieldName} trop court (${len} < ${limit.min})`,
    );
  }
  if (len > limit.max) {
    errors.push(
      `R6: Slide ${slideIndex} ${fieldName} trop long (${len} > ${limit.max})`,
    );
  }
}
