...
export async function callAIWithFallback(
  ...
  for (const provider of providers) {
    try {
      ...
      const enrichedMessages = buildMessagesForProvider(
        messages,
        context,
        provider,
      );
      ...
      if (tools && tools.length > 0) {
        if (iterationCount === 0) {
          ...
          console.log(
            "[AI Orchestrator] Forcing tool_choice: classify_intent (iteration 0)",
          );
        } else {
          toolChoice = "auto";
        }
      ...
      if (!response.ok) {
        ...
        console.warn(
          `[AI Orchestrator] ${provider} failed (${response.status}): ${errorText}`,
        );
...
${
    fontsText
      ? fontsText.split(", ").map((line) => `  • ${line}`).join("\n")
      : "  • (Non définie)"
  }
...
export function enrichPromptWithBrandKit(
  ...
  const colors = brandKit.colors || brandKit.palette || [];
  const colorHex = colors.map((c: any) =>
    typeof c === "string" ? c : c.hex || c.value
  ).filter(Boolean);
  const fontsText = normalizeFonts(brandKit.fonts);
  ...
  const colorInstruction = colorDescriptions.length > 0
    ? `Incorporate these colors naturally into the composition: ${
      colorDescriptions.join(", ")
    }.`
    : "Use professional neutral color palette.";

  const styleInstruction = brandKit.style || brandKit.voice ||
    "modern professional";
  const nicheContext = brandKit.niche
    ? ` reflecting ${brandKit.niche} industry aesthetics`
    : "";

  return `${basePrompt}

VISUAL DIRECTION (CRITICAL: This is styling metadata, NOT content to display):
Apply ${styleInstruction} visual style${nicheContext}. ${colorInstruction} Use high-quality professional composition with strong visual hierarchy and readability. Typography should feel ${
    fontsText || "clean and modern"
  }. Create cohesive brand-aligned imagery.
