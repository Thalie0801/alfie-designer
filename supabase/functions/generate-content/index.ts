...
serve(async (req) => {
  ...
  try {
    const { type, prompt, brandKit, duration, aspectRatio }:
    ...
    if (type === "image") {
      ...
      if (brandKit?.palette && brandKit.palette.length > 0) {
        fullPrompt += `. Use these brand colors: ${
          brandKit.palette.join(", ")
        }. Create a professional social media ready design.`;
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 402) {
          return new Response(
            JSON.stringify({
              error: "Crédits insuffisants sur ton workspace Lovable.",
            }),
            {
              status: 402,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      return new Response(
        JSON.stringify({
          error:
            "La génération vidéo VEO3 n'est pas encore disponible. Elle sera ajoutée prochainement ! 🎥",
          status: "coming_soon",
        }),
        {
          status: 503,
