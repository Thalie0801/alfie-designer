...
serve(async (req) => {
  ...
  try {
    ...
    if (typeof parsed.quota_visuals_per_month === "number") {
      updates["quota_visuals_per_month"] = parsed.quota_visuals_per_month;
    }
    if (typeof parsed.quota_brands === "number") {
      updates["quota_brands"] = parsed.quota_brands;
    }
    if (typeof parsed.quota_videos === "number") {
      updates["quota_videos"] = parsed.quota_videos;
    }
    if (typeof parsed.quota_woofs === "number") {
      updates["quota_woofs"] = parsed.quota_woofs;
    }

    if (Object.keys(updates).length) {
      const { error: uqErr } = await admin.from("profiles").update(updates).eq(
        "id",
        parsed.user_id,
      );
