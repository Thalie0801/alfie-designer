...
serve(async (req) => {
  ...
  try {
    ...
    if (created?.user?.id) {
      console.log(
        "[admin-create-user] User created successfully:",
        created.user.id,
      );
      ...
      if (
        createErr?.status === 422 || msg.includes("already") ||
        msg.includes("registered")
      ) {
        console.log(
          "[admin-create-user] User already exists, fetching existing user",
        );
    ...
    if (parsed.sendInvite) {
      try {
        ...
        console.warn(
          "[admin-create-user] Error sending invitation (non-blocking):",
          err,
        );
      }
    }
