...
serve(async (req) => {
  ...
  try {
    ...
    const rawJobId = jobRecord.id as string | number | null | undefined;
    jobId = typeof rawJobId === "string" ? rawJobId : typeof rawJobId === "number" ? String(rawJobId) : null;
    const rawShortId = jobRecord.short_id as string | null | undefined;
    jobShortId = typeof rawShortId === "string" && rawShortId.trim().length > 0 ? rawShortId : null;

    if (!jobId) {
      console.warn("⚠️ [Jobs] Insert returned without an id, job tracking disabled for this run");
    }
    ...
    try {
      ...
      if (kieResponse.ok) {
        ...
        if (generation.data?.taskId) {
          ...
          if (jobShortId) {
            responsePayload.jobShortId = jobShortId;
          }
