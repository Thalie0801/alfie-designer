...
serve(async (req) => {
  ...
  try {
    ...
    const carouselsSheet = Object.entries(carouselGroups).flatMap(
      ([carouselId, slides]: [string, any]) => {
    ...
    console.log(
      `✅ [Excel Export] Generated Excel with ${
        assets?.length || 0
      } assets, ${carouselsSheet.length} carousel slides, ${
        logs?.length || 0
      } logs`,
    );
    ...
    return new Response(excelBuffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="assets_export_${
