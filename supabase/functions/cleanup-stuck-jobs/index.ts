...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `⚠️ [Cleanup] Found ${stuckJobs.length} stuck jobs:`,
      stuckJobs.map((j) => ({
        id: j.id.slice(0, 8),
        index: j.index_in_set,
    ...
    console.log(
      `✅ [Cleanup] Marked ${updatedJobs?.length || 0} jobs as failed`,
    );

    // Mettre à jour les statuts des job_sets affectés
    const jobSetIds = [...new Set(stuckJobs.map((j) => j.job_set_id))];
    console.log(
      `📦 [Cleanup] Updating ${jobSetIds.length} affected job_sets...`,
    );
    ...
    for (const jobSetId of jobSetIds) {
      ...
      console.log(
        `✅ [Cleanup] job_set ${jobSetId.slice(0, 8)}... → ${jobSetStatus}`,
      );
    }
