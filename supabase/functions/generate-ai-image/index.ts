...
serve(async (req) => {
  ...
  try {
    ...
    if (!brandId) {
      return new Response(
        JSON.stringify({
          error: "No active brand selected. Please select a brand.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    const { data: brandKit } = await supabaseClient
      .from("brand_kits")
      .select(
        "primary_color, secondary_color, accent_color, logo_url, brand_voice",
      )
      .eq("brand_id", brandId)
      .maybeSingle();
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          error: quotaError instanceof Error
            ? quotaError.message
            : "Quota exhausted",
        }),
        {
    ...
    const reformulatePrompt = (userPrompt: string): string => {
      ...
      if (
        /^(fais|fait|faire|cr[éeè]e|g[éeè]n[éeè]re|je veux|donne|montre)/i.test(
          userPrompt,
        )
      ) {
        return userPrompt
          .replace(
            /^(fais|fait|faire|cr[éeè]e|g[éeè]n[éeè]re|je veux|donne|montre)(-moi)?/i,
            "",
          )
    ...
    const enhancedPrompt =
      `Professional marketing visual: ${basePrompt}. ${brandContext}${
        ratioDescriptions[aspectRatio] || ratioDescriptions["1:1"]
      }. High quality, clean composition, brand-ready design.`;
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return new Response(
          JSON.stringify({
            error:
              "Limite de génération atteinte. Réessayez dans quelques instants.",
          }),
          {
            status: 429,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const generatedImageUrl = data.choices?.[0]?.message?.images?.[0]?.image_url
      ?.url;

    if (!generatedImageUrl) {
      console.error(
        "No image URL found in response. Full response:",
        JSON.stringify(data),
      );
      return new Response(
        JSON.stringify({
          error:
            "L'IA n'a pas généré d'image. Essayez de reformuler votre demande ou réessayez dans quelques instants.",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    try {
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        if (binary) {
          const filePath =
            `generated/${user.id}/${brandId}/${Date.now()}-${crypto.randomUUID()}.${ext}`;
      ...
      console.warn(
        "Failed to persist generated image to media-generations bucket",
        storageError,
      );
    }
