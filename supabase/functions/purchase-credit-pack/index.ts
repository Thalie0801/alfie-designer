...
serve(async (req) => {
  ...
  try {
    ...
    const isEligibleForDiscount =
      (profile?.plan === "studio" || profile?.plan === "pro") &&
      pack.name === "Pack 50 crédits";
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: pack.name,
              description:
                `${pack.credits} crédits IA pour générer des visuels`,
            },
      ...
      mode: "payment",
      success_url:
        `${origin}/credit-purchase-success?session_id={CHECKOUT_SESSION_ID}`,
