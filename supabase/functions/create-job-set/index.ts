...
serve(async (req) => {
  ...
  try {
    ...
    if (brandOwnership.user_id !== user.id) {
      console.error(
        `[create-job-set] ⛔ User ${user.id} tried to access brand ${brandId} owned by ${brandOwnership.user_id}`,
      );
      return new Response(
        JSON.stringify({
    ...
    console.log(
      `[create-job-set] ✅ Brand ownership verified for user ${user.id}`,
    );
    ...
    const jobSet = await withIdempotency(idempotencyKey, async () => {
      console.log(
        `[create-job-set] Starting for brand ${brandId}, count=${normalizedCount}, aspect=${aspectRatio}`,
      );
      ...
      if (quotaErr || !quotaResult?.[0]?.success) {
        console.error(
          "[create-job-set] ❌ Quota reservation failed:",
      ...
      console.log(
        `[create-job-set] ✅ Quotas reserved: ${normalizedCount} visuals for brand ${brandId}`,
      );
      ...
      const { data: planResponse, error: planErr } = await supabase.functions
        .invoke("alfie-plan-carousel", {
          body: {
            prompt,
      ...
      if (planErr || !planResponse?.plan?.slides) {
        console.error(
          "[create-job-set] ❌ Planning failed:",
          planErr || "No slides in response",
        );
        // Refund quotas
      ...
      const jobsData = plan.slides.slice(0, normalizedCount).map(
        (slide: any, i: number) => {
          ...
          if (slide.kpis && slide.kpis.length > 0) {
            const kpiText = slide.kpis.map((k: any) => `${k.label} ${k.delta}`)
              .join(", ");
          ...
          const styleHints: Record<string, string> = {
            ...
            impact:
              "Visual with data visualization elements, metrics, success indicators",
      ...
      console.log(
        `[create-job-set] 🎨 Inserting ${jobsData.length} jobs into database...`,
      );
      ...
      console.log(
        `[create-job-set] ✅ Successfully inserted ${jobsData.length} jobs for job_set ${newJobSet.id}`,
      );
