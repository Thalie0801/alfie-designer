...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[admin-delete-user] Suppression de l'utilisateur: ${targetUserId}`,
    );
    ...
    if (!getUserError && userData?.user) {
      authUserExists = true;
      console.log(
        `[admin-delete-user] Utilisateur auth trouvé: ${userData.user.email}`,
      );
    } else {
      console.log(
        `[admin-delete-user] Utilisateur auth non trouvé (probablement déjà supprimé)`,
      );
    }
    ...
    if (affiliateError) {
      console.error(
        "[admin-delete-user] Erreur suppression affiliates:",
        affiliateError,
      );
    } else {
    ...
    if (profileError) {
      console.error(
        "[admin-delete-user] Erreur suppression profil:",
        profileError,
      );
      return json({
        error: `Erreur suppression profil: ${profileError.message}`,
      }, 500);
    }
    ...
    if (authUserExists) {
      ...
      if (authError) {
        console.error(
          "[admin-delete-user] Erreur suppression compte auth:",
          authError,
        );
        return json({
          error: `Erreur suppression compte: ${authError.message}`,
        }, 500);
      }
    ...
    console.log(
      `[admin-delete-user] Éléments supprimés: ${deletedItems.join(", ")}`,
    );
