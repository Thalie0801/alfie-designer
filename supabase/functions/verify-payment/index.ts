...
import { validateInput, VerifyPaymentSchema } from "../_shared/validation.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (existingSession?.verified) {
      console.warn(
        `⚠️ Payment replay attempt detected for session: ${session_id}`,
      );
      return new Response(
        JSON.stringify({
    ...
    if (brandName && userId) {
      console.log(
        `🎨 Processing brand purchase for user ${userId}, brand: ${brandName}`,
      );
    ...
    console.log(
      `✅ Payment session stored for ${customerEmail}, plan: ${plan}`,
    );
